...
const Pokemon = ({ id, url, name, picture }: propsData) => {
  ...
  const informationCard = () => {
    if (dataPokemon) {
      ...
      const getAbilities = dataPokemon.abilities.map(
        (ability) => ability.ability.name
      );
      const getExperience = dataPokemon.base_experience;
      const getheight = dataPokemon.height;
      const getMoves = dataPokemon.moves.map(
        (movements) => movements.move.name
      );
                            ...
                            <GraphicStat>
                              {stats.max_stat.map((number) => (
                                <div
                                  key={number}
                                  className={
                                    number >= stats.base_stat + 1
                                      ? "global stat"
                                      : "stat propertyPokemon"
                                  }
                                ></div>
                              ))}
                            </GraphicStat>
                  ...
                  <FirstBlockInRightDiv>
                    <UlFB>
                      ...
                      <Li>
                        ...
                        <PPokemonFB>
                          {information && information.height}
                        </PPokemonFB>
                      </Li>
                      <Li>
